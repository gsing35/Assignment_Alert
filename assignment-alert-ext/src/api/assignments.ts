...
export async function getUpcomingAssignments(): Promise<AssignmentResponseDTO[]> {
    ...
    const response = await fetchWithTimeout(`${BASE_URL}/api/v1/assignments?userId=${userId}&filter=upcoming`)  // wait for backend to response when fetching upcoming assignments
...
export async function getIncompleteAssignments(): Promise<AssignmentResponseDTO[]> {
    const userId = await requireUserId()
    const response = await fetchWithTimeout(`${BASE_URL}/api/v1/assignments?userId=${userId}&filter=incomplete`)    // wait for backend to response when fetching upcoming assignments
    return handleResponse<AssignmentResponseDTO[]>(response)
}

export async function getBlockingAssignments(): Promise<AssignmentResponseDTO[]> {
    const userId = await requireUserId()
    const response = await fetchWithTimeout(`${BASE_URL}/api/v1/assignments?userId=${userId}&filter=blocking`)  // wait for backend to response when fetching upcoming assignments
    return handleResponse<AssignmentResponseDTO[]>(response)
}

export async function getAssignmentsByCourse(courseId: number): Promise<AssignmentResponseDTO[]> {
    const userId = await requireUserId()
    const response = await fetchWithTimeout(`${BASE_URL}/api/v1/assignments?userId=${userId}&courseId=${courseId}`) // wait for backend to response when fetching upcoming assignments
    return handleResponse<AssignmentResponseDTO[]>(response)
}
...
export async function getAssignmentById(assignmentId: number): Promise<AssignmentResponseDTO> {
    const userId = await requireUserId()
    const response = await fetchWithTimeout(`${BASE_URL}/api/v1/assignments/${assignmentId}?userId=${userId}`)
    return handleResponse<AssignmentResponseDTO>(response)
}
...
export async function getAssignmentsByPriority(priority: Priority): Promise<AssignmentResponseDTO[]> {
    const userId = await requireUserId()
    const response = await fetchWithTimeout(`${BASE_URL}/api/v1/assignments?priority=${priority}&userId=${userId}`)  // wait for backend to response when fetching upcoming assignments
    return handleResponse<AssignmentResponseDTO[]>(response)
}