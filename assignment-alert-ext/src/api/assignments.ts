...
export async function getUpcomingAssignments(): Promise<AssignmentResponseDTO[]> {
    const response = await fetchWithTimeout(`${BASE_URL}/api/v1/assignments?filter=upcoming`)  // wait for backend to response when fetching upcoming assignments
...
export async function getIncompleteAssignments(): Promise<AssignmentResponseDTO[]> {
    const response = await fetchWithTimeout(`${BASE_URL}/api/v1/assignments?filter=incomplete`)    // wait for backend to response when fetching upcoming assignments
    return handleResponse<AssignmentResponseDTO[]>(response)
}

export async function getBlockingAssignments(): Promise<AssignmentResponseDTO[]> {
    const response = await fetchWithTimeout(`${BASE_URL}/api/v1/assignments?filter=blocking`)  // wait for backend to response when fetching upcoming assignments
    return handleResponse<AssignmentResponseDTO[]>(response)
}

export async function getAssignmentsByCourse(courseId: number): Promise<AssignmentResponseDTO[]> {
    const response = await fetchWithTimeout(`${BASE_URL}/api/v1/assignments?courseId=${courseId}`) // wait for backend to response when fetching upcoming assignments
    return handleResponse<AssignmentResponseDTO[]>(response)
}
...
export async function getAssignmentById(assignmentId: number): Promise<AssignmentResponseDTO> {
    const response = await fetchWithTimeout(`${BASE_URL}/api/v1/assignments/${assignmentId}`)
    return handleResponse<AssignmentResponseDTO>(response)
}
...
export async function getAssignmentsByPriority(priority: Priority): Promise<AssignmentResponseDTO[]> {
    const response = await fetchWithTimeout(`${BASE_URL}/api/v1/assignments?priority=${priority}`)  // wait for backend to response when fetching upcoming assignments
    return handleResponse<AssignmentResponseDTO[]>(response)
}
